...
**The `thisType` slot, described three ways.** The specification names it `thisType` and says it holds a type object; the engine spells it `this` and holds a reflection *node*, making it the only type-valued slot on a signature that is not already a type object; and the absent case answers `any` in the engine and in §6.3, against *never* in the kit's own table. One spelling and one absent-case answer should win in all three.
