...
**The evaluability rule, enforced — with its library half still open.** [#sec-object-types](https://sirisian.github.io/proposal-runtime-types/#sec-object-types) requires a member's initializer to be compile-time evaluable “and it is a type error otherwise”, and for a while nothing checked it. It is checked now. [#annex-evaluable-fragment](https://sirisian.github.io/proposal-runtime-types/#annex-evaluable-fragment) defines the fragment *subtractively* — “the ECMAScript grammar less the following” — which makes the membership test a walk with a rejection list rather than an evaluator: the suspension forms, class declarations and expressions, `eval` by name so the indirect form is caught, the `Function` constructor, `Proxy`, dynamic `import()`, and `debugger`.
