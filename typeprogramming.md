...
The clause leaves the format open, so building the scheme fixed it, and the decisions are recorded here rather than only in the engine. It is a **table**, as the clause's own words say — “a serialization of the interning table” — and nothing else works: a primitive record refers to *itself*, since `getReflection(uint8).type` is `uint8`, and a recursive type closes a cycle through a nested position, so a serializer that walks by containment does not terminate on the simplest type in the language. An entry names another by index; the reader allocates every entry before filling any, which is what interning already does to tie a knot.
...
What remains is the **hash over a module graph** — sources transitively plus the specifiers that resolved them, since a rename resolving elsewhere changes the graph even when both files are unchanged — and a **producer**, which needs one thing the scheme does not supply: resolving a nominal's declaration to a module specifier. The consumer waits on a tool that wants it. The specification still does not fix the format, which is now a live choice rather than a deferral: either the clause records what a producer emits, or every tool reads the reference implementation instead of the clause.
