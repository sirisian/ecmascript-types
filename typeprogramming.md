...
What the clause still describes and nothing builds is the **documentation rider** — the text at each origin, which an artifact also carries, since “a dependency's sources are exactly what a consumer does not have on disk”.
...
What remains is the wire. A published dependency ships built output and no sources, so an origin points at a file that is not there and the text has to travel — which needs the R20 artifact, unimplemented and with its format deliberately unfixed. Three questions wait on it: carry raw comment text rather than a structured subset, so that a consumer which cannot parse a comment shows nothing and is still correct, as an artifact that is “an optimization and never a semantics” requires; carry *every* origin's text and let the tool choose, since choosing the first declared is the load-order crowning that kept documentation out of the node model; and key the text in an artifact-local table rather than by path, so it cannot go stale relative to the hash the artifact contract already checks.
