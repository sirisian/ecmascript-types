...
**R15 — Add declared, verified inverses, last and opt-in.** An `@inverse(fn)` decorator on the builder declaration — standalone functions are already decoratable with a `FunctionMetadata` slot in [decorators.md](decorators.md) — rather than any imperative registry: the association is part of the declaration, has one possible author, and cannot vary with module evaluation order. Consulted only after ordinary inference and R14 trials fail; the proposal is confirmed by forward evaluation and assignability before it binds, so an incorrect inverse can only fail inference, never mislead it. Multi-slot builders propose via a record keyed by parameter name, verified jointly. Four details complete it: for a parameter reached through a rest, the inverse receives the tuple of the collected arguments' types and returns the pack's tuple, one rule for scalars and packs; a proposal is a type object or an object of type objects keyed by parameter name, anything else refused naming the builder; on an overloaded builder, the inverse consulted is the resolved member's; and the decorator's primitive accepts only the live decoration context of the declaration being decorated, which is what makes the association a fact of the declaration rather than a registration. The engine implements it as `import { inverse } from 'std:types'` applied `@inverse(fn)` to a builder declaration, with the refusal for a builder that declares none naming the builder and the parameter.
