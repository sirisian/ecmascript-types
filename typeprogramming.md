...
**`fn`'s parameter records.** The kit's function constructor takes bare parameter types, so the record fields (`rest`, `optional`, `initial`, names) and a signature's `thisType` are reachable only through `Reflect.makeType` — which is what challenges 17 and 462 do. Nothing is unreachable; `makeType` is the general constructor and `fn` is the common case's sugar, the same relationship `deepMap` has to `traverse`. Whether the sugar should widen to take full parameter records alongside bare types is a surface question rather than a capability one. Decide with usage.

**The corpus document has no published home.** This section and §4 cite [typechallenges.md](typechallenges.md) four times — for the 190 machine-checked programs, the frequency counts behind §4.0's corpus-driven additions, and the challenges named in §4.12 — and no such file exists in this repository or in its history. The corpus itself is real: the engine runs it under `test/engine262/runtime-types/corpus/type-challenges/`, and the specification's evaluable-fragment annex refers to it and to its eleventh challenge. What is missing is the design document those links point at, so every claim this document makes on the corpus's authority is currently uncheckable by a reader. Either publish it here or repoint the links at wherever it lives.
