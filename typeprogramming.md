...
What remains is the **hash itself**, which the clause deliberately does not fix — the producer emits the inventory a key is taken over, and a semantics identifier beside it, so a consumer can tell a stale dependency from a version skew. And the **consumer**, which waits on a tool that wants one.
