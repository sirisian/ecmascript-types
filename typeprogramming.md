...
- **P1 — Deferral.** `Partial<T>` with `T` unresolved is a real type the checker can carry, compare for identity, and instantiate later. *Builder answer:* an unevaluated call `partial(T)` is likewise carried as an interned deferred computation — identical calls are the identical type object — and evaluates at specialization. The core operators are carried the same way: `keyof T` and `T[K]` over an unbound `T` are deferred computations of one kind with a builder call, identified by operator and operands, opaque until specialization closes them, and bounded meanwhile by what the operator promises (`keyof T` by the key universe, `T[K]` by `T[keyof T]`, a call by the builder's declared return). One kind rather than one per operator, so that identity, evaluation and opacity are stated once; rustc reached the same shape with its `Alias` kind. What is lost relative to TypeScript is any reasoning *finer than identity* before specialization; §5.2 examines the consequences.
...
**R8 — `Reflect.matchType` and `Reflect.inferSlot`.** Specified ([#sec-structural-matching](https://sirisian.github.io/proposal-runtime-types/#sec-structural-matching)), **implemented**, and the deferral is over. The optionality argument has changed and the clause records it: the measurement was that this document's own catalog needed the operation exactly zero times, and [decorators](decorators.md) that dispatch on the structure of a type they are handed — emitting a schema, a validator, documentation — cannot be written without it, with `when extends T:` in [pattern matching](patternmatching.md) as its syntactic form consuming the same unification. Patterns are structural only: a slot inside a deferred application is a type error at the match, not a `null`, because evaluating a builder against an unbound slot is the inversion nothing here performs. The implementation was smaller than the deferral implied: the unification walk already existed, binding the ~parameter~ records of a generic signature and already citing this clause, so a slot is a type parameter rather than a new kind and “a slot occurring twice binds once” fell out of the binding map that was already there. What was missing was the reflective surface.
...
**R18 — Exact pattern subtyping on the regular fragment.** Specified and **implemented**, and tied to R7's provisional claim. The tier is decided by syntactic size against a spec-fixed automaton bound rather than by remaining fuel, so the judgment is host-independent. The size constant is 100, summed over both patterns, chosen by measuring what real patterns cost in nodes — an identifier 24, a date 29, an email 54 — and recorded beside itself. It is conservative on purpose: the syntactic size bounds the NFA and not the determinized product, so it is a bound that is the same on every host rather than a proof that the walk is cheap, which is the property the clause asks for.
...
**R20 — The expansion artifact.** Specified ([#sec-expansion-artifact](https://sirisian.github.io/proposal-runtime-types/#sec-expansion-artifact)); the **reference scheme is implemented**, the producer and the consumer are not. An optimization and never a semantics: a consumer that ignores every artifact computes the same types, and one that reads a stale artifact detects it by the hash rather than by disagreeing.

The clause **now fixes the format**, which building the scheme is what made possible — and necessary. A deferral costs nothing while nothing implements it; once something does, the choice is between recording the format and letting the reference implementation become the specification by default. The clause's own argument decides it: an artifact is worth having because a tool reading one is trusting that its producer computed what the specification says, which presumes *independent* producers, and a format only one implementation can emit makes that sentence false.

Four choices carry weight. Entries are the **node model** rather than a bespoke encoding, because a second definition of what a type is would have to be kept in step with the first by hand — and three defects in this work came from exactly that drifting. They are ordered by `orderKey`, the total order interning already sorts by, so the bytes do not depend on how a caller enumerated its exports. A reference is tagged `{ $ref: n }` rather than a bare index, since a literal carries a value and a property an initializer, either of which may be a number. And a **version field is written from the first entry**: a reader meeting a higher version declines, which is the path a stale hash already takes, so forward compatibility costs one comparison and only if the field was there from the start.

A **nominal type is carried by name**, never by value. Its record holds a declaration — a parse node, whose parent pointer does not terminate — and a live constructor, and those are the only two leaves a table cannot encode. Dropping nominals instead was measured and rejected: they are 15% of a realistic public surface's entries, but 58% of entries *reach* one, so eleven of twelve exported types would go with them. The declared name comes from R22's origins, which is why a class carrying no origin at all had to be fixed first — a gap found by asking this question. Reading takes a **resolver**, because a nominal names a declaration the consumer has its own copy of; a name it cannot resolve makes the reader decline, as a stale hash and a newer version already do.

A **producer exists**, and an artifact for a real two-module graph carries what it should: the exported types and not the private ones, and a nominal named by the module that declares it. Getting there took four repairs to a chain whose every link was correct locally — a record keyed on the parse tree, which is not stable across the passes that check a linking module; a record that replaced rather than merged, letting the pass that knew least decide; imported types collected from the map that holds what a name's *value* is typed as rather than what it *is*; and that map seeded into `bindings` while a type annotation resolves through `aliases`. Each was found by measurement after plausible fixes that changed nothing, which is its own lesson: when a change makes no difference rather than a different failure, doubt the observation path before the computation.

**R20 is complete.** A producer emits an artifact for a module graph, a consumer reads one back — in a different agent from the one that produced it, yielding the very Type Object that agent's own evaluation produces rather than a lookalike — and the clause fixes the format and the key.

The key turned up a correction to the clause rather than an addition. It named *how the hash is computed* among the things it did not fix, and said nothing about the hash's **input**, which is where interoperability lives: two producers running one digest over one inventory get different keys if they serialize it differently, and a consumer then reads every current artifact as stale — quietly, so the symptom is that artifacts never help rather than that artifacts are broken. The same defect the format had, one level down. Both are fixed now: a length-prefixed canonical encoding, and SHA-256 over it, implemented rather than imported because this engine takes no host built-ins anywhere.

The **documentation rider** is built too, so nothing the clause describes is unimplemented. One rule needed the clause widened rather than only implemented: it forbade an *open application* and said nothing about a bare unbound parameter, and measuring found that the clause's own example never reaches a record here — `mk(T)` in a return position resolves away, leaving a bare parameter, so the rule as written held without being enforced while the shape that actually occurs went unnamed. An exported generic signature is unbound for the same reason `partial(T)` is, and a name carrying one is omitted rather than the artifact refused.
...
**The evaluability rule, enforced — both halves.** [#sec-object-types](https://sirisian.github.io/proposal-runtime-types/#sec-object-types) requires a member's initializer to be compile-time evaluable “and it is a type error otherwise”, and for a while nothing checked it. Both halves of [#annex-evaluable-fragment](https://sirisian.github.io/proposal-runtime-types/#annex-evaluable-fragment) are checked now. The *syntactic* half is the grammar less a rejection list — the suspension forms, classes, `eval` by name so the indirect form is caught, the `Function` constructor, `Proxy`, dynamic `import()`, and `debugger` — which makes it a walk rather than an evaluator. The *library* half is enforced on the FUNCTION rather than on the call syntax, because a denylist over source text sees `Math.random()` and misses a binding holding it, a computed member access, or an element of an array; C++ marks `constexpr` and Rust `const fn` the same way. The floor the annex guarantees is admitted by construction, since only the exclusions are marked.

Two things remain. The annex says its conformance suite fixes the exact enumeration, so the exclusion list is a denylist that *grows*: an impure built-in nobody has named is still accepted. And the two spellings refuse at different moments — an object type is resolved where it is written, so a bad default there is an early error, while an interface is resolved when it is *used*, so an unused one is refused as its declaration runs. Both refuse; only the first is static.

What is still open is the annex's *library* rule: a built-in is in the fragment when its result is determined by its arguments, it mutates nothing outside values the call created, and it depends on no host or ambient state. That is a judgement about built-ins rather than about syntax, and nothing decides it, so a pure-looking call to an impure built-in still passes. And the two spellings refuse at different moments: an object type is resolved where it is written, so a bad default there is an early error, while an interface is resolved when it is *used*, so an unused one is refused as its declaration runs. Both refuse; only the first is static.

**Contract vocabulary creep — answered, and a mechanism withdrawn.** R13 allows any evaluable predicate, and the question was whether practice would demand more. Measured against the kit's 34 contracts, the vocabulary is wider than the phrase “a proposition about one evaluation” suggests: a clause may quantify over its argument's members, relate the argument to the result, and call other builders, because the argument is a value at evaluation. What it cannot do is state a claim proven once for **all** arguments, which is why variance, parametricity and the algebraic laws — idempotence, commutativity, `required(partial(T)) === T` — stay out of reach. Nothing in the kit or the 190-program corpus asked for them, and the two systems that do allow such claims (Dafny, Liquid Haskell) both carry a solver, which [#annex-evaluable-fragment](https://sirisian.github.io/proposal-runtime-types/#annex-evaluable-fragment) puts outside this proposal.

The gap that *is* real is narrow: a contract on a builder nothing ever applies is verified nowhere while the assumed half still travels to a consumer. An `exemplars` decorator forcing specialization at named arguments was drafted, implemented and then withdrawn, because that case is inert — a contract that misleads only code which is never specialized misleads only code that never runs, and any concrete call verifies it. What closes it is a test calling the builder, which is a library author's ordinary obligation. Two defects the attempt exposed were kept: decorators on an exported function declaration reached an assertion that a decorated export is always a class, and two subtype folds were answering the intern table's question of whether two records *are* the same, so an object record matched an interned tuple and `Reflect.makeType` returned the wrong kind.

**Contracts on the kit — written, and now visible across a module.** §6.2 asked for them and the kit carried none, which left R13 with no user. All 55 type-returning exports are now accounted for: 34 state a contract and 21 record why they do not, beside the definition, so a later pass can tell deliberate from overlooked. The bounds are the variance each builder has — a widening leaves the argument assignable to the result, a narrowing the reverse, dropping members leaves a supertype — and where no subtyping bound exists a builder states the *kind* it produces instead, as a disjunction with `never`, since canonicalization collapses a composite whose required component is uninhabited.

Writing them was worth more than the contracts themselves: three bounds went in too strong and the corpus refused each within a minute. `merge` measured on disjoint keys claimed both arguments, and with overlapping keys B wins the shared member so the result satisfies B alone. `flatten` was grouped with the sequence builders by name and actually unwraps an array to its element. And the kind facts were written without `never` in view. The working rule that came out of it: *pick the input that would falsify the claim, not the one that shows it off.*

The assumed half of a contract also crosses a module boundary now, so those bounds reach a consumer's generic body rather than only its concrete calls — the `sanitize<T>` case §6.2 is written for. An imported builder's declaration travels the channel that already carries what an imported name *is*.

**The `pattern` meta key — shipped, and the withholding argument was wrong.** R7's claim was provisional and `suffixed` and `stringPattern` were written and unexported. The reason given was that reflexive-only subtyping makes two equivalent patterns unrelated types; Rust weakens that considerably, having no subtyping at all while its validated-newtype pattern is used heavily. The reason then given was the *namespace*: `pattern` is a key in a flat, first-come space and a shipped export cements the claim.

Measured, that reason does not hold either. `StringPattern` is a hardcoded intrinsic that **already** claims the key in every realm, and a user `meta` block claiming it is refused — so withholding preserved nothing and the name cannot be returned either way. What is genuinely reversible is the subtype judgment, which stays at the floor of reflexivity and can be strengthened by R18 without breaking anyone. All three are exported now, `prefixed` among them as `suffixed`'s mirror.

Being unexported is what hid a defect, and that is the lasting lesson rather than the naming one. All three build their type through `makeType` with a RegExp, which walked it as an ordinary object and flattened it to `{ lastIndex: 0 }` — so none of them worked, the type they produced was not the one the annotation produces, and the round trip `#sec-reflect-maketype` exists to keep was broken for pattern metadata. Nothing ran them, so nothing found it. Written code kept unexported is untested code kept in the tree.
...
**Documentation across the wire — the first-party half closed, the wire half still gated.** R22's origins are declaration references, and a hover wants two things: where a member was declared, and the text written there.

The first now exists for a member and not only for a type. [#sec-provenance](https://sirisian.github.io/proposal-runtime-types/#sec-provenance) says “a Property Type Record **and** a Type Record may carry an [[Origin]]”, and only the Type Record half was recorded — so `type User = { name: string }` could say where *User* was declared and nothing about `name`, which is the question the clause's own motivation asks. Members now carry their own positions, keyed on the interned Type Object because canonicalization rebuilds property records, which also makes the specified union fall out: two structurally identical declarations reach one Type Object and both sites land on the member.

The wire half is closed too. An artifact carries the text written above each exported type, and all three questions were answered as recorded: raw comment text rather than a structured subset, so a consumer that cannot parse one shows nothing and is still correct; *every* origin's text rather than the first, since choosing the first declared is the load-order crowning that kept documentation out of the node model to begin with; and an artifact-local table rather than a path, so it cannot go stale relative to the key the contract already checks. Both comment forms travel, delimiters included, and a type with no comment above it carries none rather than an empty one — documented as nothing and not documented are different, and only the second is true.

One claim in the clause is not true of the engine as it stands. The fallback for a minted record is “the application that produced it, which is the expression a diagnostic already names”, and diagnostics name the computed form instead: a value refused against `mkPartial(User)` reports `is not assignable to "{}"`. A tool can still name the application, since it is the expression being displayed and a tool has the whole program in view — so nothing is blocked — but the parenthetical describes a diagnostic that would have to be written.

**Metadata inside walks — answered in part.** A walk that passes a parameterization through untouched is safe, and that is the current default: a field's type rides as a type object and a walk's default arm passes it through, so `deepPartial`, `traverse` and `deepMap` are fine over a struct containing a branded or enum field. What is not free is a walk that *rebuilds* one, and the engine's history here is the argument for R1's identity law: a metadata record once reached the write side as an ordinary object and was reconstructed one level deep, so a pattern's structural marker came back an object rather than a record and the rebuilt type was a *second* type object for the same type — correct to every question except `===`. Reconstruction is structural to full depth now. What is still open is whether a meta type should be able to veto a structural edit where it must: a builder should not strip `nonZero` from a type used as a divisor without the change being an ordinary, checked assignability question, which it is. What was still open — whether a meta type should be able to VETO a structural edit — is answered no, and measured. A builder can strip a mark: `strip(Marked)` reaching for the reflected `base` returns `float64`, and the result is not the marked type. It does not need a veto to be safe, because dropping the mark is already an ordinary assignability question and the judgment answers it: a marked type is assignable to its base, a stripped one is **not** assignable back to the marked type, and a function taking the marked type refuses the stripped one at the call. A veto would add a second mechanism to refuse what the first already refuses, and would have to decide what a builder is allowed to do rather than what a value is allowed to be — which is the distinction R1's identity law rests on.
