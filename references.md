...
Rebinding requires a mutable reference binding: `let ref` and reference parameters qualify; ordinary bindings and `const ref` do not. A known invalid destination is an early type error even inside an unused function. The checker follows the nearest lexical declaration, including destructuring, loop bindings and closures, independently of the referent's type. Unknown destinations, including names an object environment may intercept, retain the runtime check.
