...
`first(a)++` works because a call that returns a `ref` is not decayed in a position that consumes a *location*. There are three such positions: the operand of `++` or `--`, the operand of a `ref` argument, and an assignment target. The second re-borrows, so `g(ref first(a))` passes the location the call returned straight on. Everywhere else the returned reference decays as usual — `let v = first(a)` copies the element's value, and `typeof first(a)` is the element's type. A call in one of these positions whose return type is not a `ref` type is refused before the program runs where the type is known, and is a TypeError at the operation where it is not.
