...
Inside a generic class's body, its bare name in a type position means the class over its own parameters, as `this` does, so a fluent method writes `begin(): PacketWriter { return this; }` whatever the writer's `Size`; outside the body, the bare name means the defaults. A static member's parameters are those of the specialization it is reached through.
...
An owner may be declared without a body, as an abstract method is, and then an application that no attached overload matches is an error rather than a fallback. A bodyless owner needs an attached overload in its own declaration group, since nothing else could implement it; in an abstract class, a bodyless generic method with no attached overload is an abstract member, as any bodyless method there is. A generic body can forward an open argument to it, checked once against the owner's signature and selected per specialization:
