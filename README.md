...
At an explicit `await`, a participating operand whose selected callable `then` cannot accept the two resolving callbacks is an early type error. The callbacks are ordinary function values, passed by value; a required reference parameter, an incompatible scalar parameter, or an unsatisfied required slot can prove failure. Rest, optional and default parameters use the actual two-argument mapping. Every possible selected signature must fail: an unknown callback contract, generic or viable alternative prevents rejection. A non-callable `then` is a valid ordinary value to await, and a called method's normal result is ignored rather than treated as the awaited result. The intrinsic Promise path is preserved. This query neither executes the method nor infers arbitrary thenable results, and changes no promise scheduling, reference decay or liveness rule.
