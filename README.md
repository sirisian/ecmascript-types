...
A participating typed `instanceof` target also checks its effective `Symbol.hasInstance` contract. A present hook that cannot be called, or whose known signatures cannot accept the actual left operand, establishes an early error when every selected alternative fails. The hook is called even for a primitive left operand. Its result need not be Boolean: an Object result is truthy. Nullish hooks select ordinary callable-target fallback. An optional hook prevents a closed negative proof unless that fallback also fails under the rule below; an open callable object or a viable union alternative prevents rejection. Checking invokes no hook or getter and uses the actual well-known Symbol identity, including inherited and specialized contracts.

When `instanceof` selects the ordinary fallback, a participating right operand is also rejected early if its creation origin proves that it has no call capability. Both facts are required: a nullish effective `Symbol.hasInstance` and a proved ordinary object origin, such as a stable binding initialized by an object literal. An open structural type or a class name alone does not prove non-callability. A missing hook declaration, a callable custom-hook possibility, an unknown factory, or a replacement whose origin is not fixed keeps the runtime judgment. The check evaluates no hook, getter or reference, and a primitive left operand does not skip this fallback requirement.
...
An ordinary property operation rejects a participating receiver whose type contains only `null` and `undefined`. This covers named and computed reads, writes, updates, deletion and calls: `function f(x: null) { x.a; }` is an early error. Boxing other primitives remains valid. A skipped `x?.a.b` does not perform those accesses, while `(x?.a).b` checks the reached outer receiver. Existing declared-member checks on mixed unions still apply. Untyped and `any` receivers keep runtime timing; the check neither reads a reference nor removes runtime liveness checks.
...
A field whose type is a value type class is stored inline by default, so a bare field of such a type embeds a copy. A subclass instance may not be assigned to one at all: the base's storage is its own width and a subclass that adds storage does not fit, so the store is refused rather than silently reduced to the base slice. The subtype relation is untouched — a subclass still satisfies ```is``` and ```instanceof```, and a bound ```T extends Base``` carries the whole value — and ```subclass := Base``` is the explicit widening where the base part is actually wanted. An optional one, ```T | null```, is inline too: a discriminant and the payload, defined in [memory layout](memorylayout.md#optional-values), so ```[10].<A>``` is ten instances and ```[10].<A | null>``` is ten optional instances, both contiguous. Closing a recursive cycle is therefore not a property of the field but of the class: a class that contains itself is declared a ```reference class```, whose instances are held and passed by reference and whose fields alias. ```sealed``` and ```abstract``` classes are reference types already, so a field of one holds any subclass directly and needs no modifier:
...
A write to a typed array's `length` is a typed count boundary too. A literal takes `uint64` context, so `a.length = 3` is valid for a growable array; `a.length = "3"`, a negative or fractional literal, and a known `number` binding are early type errors. Write an explicit conversion for a known numeric value of another type. An `any` value is checked at runtime: a numeric value must be exactly representable, and a nonnumeric value is a TypeError. Untyped arrays retain JavaScript's existing length coercion.

A literal length that contradicts a fixed extent is an early type error, even in an unused function. A compatible count does not waive the [reference-liveness rules](references.md): resizing during a `ref` loop still fails, and removing a borrowed element still invalidates that borrow.
...
The built-in unsigned right shift `>>>` is unavailable for ECMAScript `bigint`, even when both operands are BigInt. A participating `bigint` pair in `x >>> count` or `x >>>= count` is an early error after dispatch and literal adoption. BigInt `<<` and `>>`, Number `>>>`, and sized integer shifts such as `uint64 >>>` retain their meanings; a host BigInt payload does not make a sized integer the ECMAScript BigInt type. Compose this domain exclusion with conversion failures across unions, rejecting only when every selected path is invalid. Unknown operands and ordinary untyped BigInt expressions retain runtime checking.
...
At an explicit `await`, or at a value's implicit assimilation by an async function return (including a concise async arrow), an async-generator return, or a plain async-generator yield, a participating operand whose selected callable `then` cannot accept the two resolving callbacks is an early type error. The callbacks are ordinary function values, passed by value; a required reference parameter, an incompatible scalar parameter, or an unsatisfied required slot can prove failure. Rest, optional and default parameters use the actual two-argument mapping. Every possible selected signature must fail: an unknown callback contract, generic or viable alternative prevents rejection. A non-callable `then` is a valid ordinary value to await, and a called method's normal result is ignored rather than treated as the awaited result. The intrinsic Promise path is preserved. Apply the query only to the value entering that consumer: a synchronous return/yield does not assimilate it, and an async `yield*` does not assimilate its raw iterable as a yielded value. Nested bodies use their own async context. This query neither executes the method nor infers arbitrary thenable results, and changes no promise scheduling, reference decay or liveness rule.
...
An explicit annotation supplies that resource type after the initializer crosses its boundary. Without an annotation, a participating initializer's known type supplies the same acquisition and disposal-call proof: `function f(r: { [Symbol.dispose]: uint8 }) { using held = r; }` is rejected without repeating the annotation on `held`. This query installs no declared storage type on the binding. Explicit `any` and broad annotations retain their boundaries, and plain untyped initializers remain dynamic. Each initializer is checked in declaration order; cleanup order, suppressed errors and reference liveness are unchanged.
...
A selected private member access is an early type error when its participating receiver is proved non-Object, including reads, stores, updates and method calls. Optional access first removes its nullish alternatives: `o: null; o?.#x` skips access, while `o: uint8 | null; o?.#x` has an impossible selected receiver. A possible Object remains eligible even when its static type does not name the declaring class; private brands can be stamped onto other Objects. This rule does not prove a brand, admit private borrowing, or change public primitive property access. Unknown and `any` receivers retain runtime brand checks.
...
A direct construction is an early type error when its target is proved to be an abstract class, including through a stable constructor alias or a specialization whose declaration is known. For example, `const K = Shape; new K()` has the same restriction as `new Shape()`. This is separate from construction capability: an abstract base remains a valid `extends` target and its constructor may run through `super()` with a concrete `NewTarget`. Reassignment, shadowing, `any`, decorators and other unknown transformations withdraw an origin proof; a factory result is not identified by its return type alone. No constructor or reference is evaluated to establish the identity.
...
Every placement argument, including nested bodies, participates in the ordinary expression checking pass in its lexical scope. For example, `new (n()) C()` is rejected early when `n` is a known non-callable typed value, just as `n()` is elsewhere. This does not add a static buffer, offset, layout or extent requirement. Argument evaluation remains exactly once in its ordinary order, with runtime placement bounds, detachment and reference-liveness checks.
...
- ```import()``` resolves to a namespace whose members keep their declared types, and deferred evaluation doesn't affect signatures. Its specifier uses implicit string conversion with the string hint. A participating specifier whose conversion is proved impossible is an early type error, including a Symbol or a selected conversion hook that cannot supply an admissible primitive. This is not a string-only input restriction: Number, null, BigInt and successful object conversions remain valid. An explicit `String(symbol)` is valid too. Unknown conversions, viable unions, ordinary JavaScript and `any` keep runtime promise-rejection timing. Checking does not invoke a loader or conversion hook. A participating options operand is also rejected early when every alternative fails either of two stages: options must be an Object or `undefined`, and its effective `with` member must be an Object or `undefined`. `null` is invalid at either stage. Inherited, specialized and getter-declared member contracts contribute without running a getter. Optional members, open views, unknown generics, `any` and viable alternatives retain runtime checks. This proof does not validate arbitrary declared attribute values: only enumerable own entries become attributes, and a type does not prove enumerability. Host attribute support and resolution remain separate runtime judgments.
