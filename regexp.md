...
```js
type RegExpExecArray<Captures: type extends [].<any> = [], Groups: type = {}, Flags: string = ''> = [string, ...Captures] & {
	index: uint32,
	input: string,
	groups: Groups
} & (Flags.includes('d') ? { indices: RegExpIndices.<Captures, Groups> } : {});

class RegExp<Captures: type extends [].<any> = [], Groups: type = {}, Flags: string = ''> {
...
```js
class String {
	match<C: type extends [].<any>, G: type, F: string>(pattern: RegExp.<C, G, F>): F.includes('g') ? [].<string> | null : RegExpExecArray.<C, G, F> | null { /* … */ return undefined; }
	matchAll<C: type extends [].<any>, G: type, F: string>(pattern: RegExp.<C, G, F>): Iterator.<RegExpExecArray.<C, G, F>> { /* … */ return undefined; }
	search(pattern: RegExp): int32 { /* … */ return 0; }
	split<C: type extends [].<any>, G: type>(pattern: RegExp.<C, G>, limit?: uint32): [].<string> { /* … */ return []; }
}
```
...
type RegExpIndices<Captures: type extends [].<any>, Groups: type> = [[uint32, uint32], ...indexPairs(Captures)] & {
...
```js
function anchored<C: type extends [].<any>, G: type>(pattern: RegExp.<C, G>): RegExp.<C, G> {
