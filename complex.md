...
`Math.sin`, `Math.cos`, `Math.tan`, `Math.log`, and `Math.pow` extend the same way. Conversions are explicit in both directions: `complex(x)` lifts a real onto the plane, `.real` projects back off it, and `complex64` and `complex128` convert between each other with an explicit cast, like their component floats. A complex has no Number value, so using one where a Number is needed — `Number(z)`, `Math.floor(z)`, `z == 3` — is a `TypeError` rather than a `NaN` or a silently dropped imaginary part; take `.real`, or `Math.abs(z)` for a magnitude. The Number questions a complex does answer are the two that classify its parts: `isNaN(z)` is `true` exactly when either part is NaN, and `isFinite(z)` exactly when both parts are finite, as Python's `cmath.isnan` and `cmath.isfinite` are.
