...
- **Type objects are opaque, and the type-record format has no named retrieval.** Given the value ```[].<uint32>```, nothing exposes "array" or ```uint32```; given ```Player | null```, nothing exposes the arms. The [type records](../typerecords.md) document defines exactly the structural encoding a walker needs - unions as ```#{ union: #[...] }```, recursion as ```recursive```/```reference```, enums as an intersection with a ```values``` record - but never names the function that yields a type's record, and fixed-extent arrays have no stated encoding at all. So this example reconstructs instantiations through combinators (```deriveArray.<E>``` rebuilding ```[].<E>``` from a parameter) instead of walking, and its one structural need - the union key - comes from the ```type``` prefix operator rather than from introspection. Two fixes would unlock walking outright: name the retrieval (```Reflect.record(t)```) and give ```[N].<T>``` its record form.
- **Live reflection and type records are one facility wearing two APIs.** Classes and enums are served by ```getReflection```; aliases, unions, and recursion are served by the record format; a serializer needs both halves and the seam runs through the middle of it. The documents already note the overlap; this example is the concrete case for merging the entry points, because a codec generator is precisely the consumer that wants fields *and* structure from one call.
