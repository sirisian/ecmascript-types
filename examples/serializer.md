...
- **The string codec is UTF-16 code units, which is a format choice worth naming.** ```str``` writes a ```uint32``` count followed by one ```uint16``` per code unit, so every ASCII character costs two bytes and a lone surrogate survives the round trip. The [UTF-8 codec](../serialization.md#strings-at-a-binary-boundary) is the other choice and trades differently: ```utf8Length``` sizes the prefix, ```toUtf8``` fills the slot, ASCII halves, and an unpaired surrogate is refused at the boundary rather than carried. Neither is the right answer for every wire format — this one keeps code units because the reader is symmetric and the count is the loop bound — but a serializer whose bytes cross a language boundary wants the UTF-8 form, and the choice should be deliberate rather than inherited from ```charCodeAt``` being the obvious loop.
