...
- The string overloads are deliberately ASCII, and the ```uint.<7>``` boundary makes that a checked guarantee: seven bits per character is the saving, and a non-ASCII character fails the cast rather than being mangled. Length-prefixed UTF-8 composes the same way with ```write.<uint8>``` per byte, using the [codec](../serialization.md#strings-at-a-binary-boundary) rather than a host ```TextEncoder``` — ```value.utf8Length``` sizes the prefix and ```value.toUtf8(slot)``` fills a byte slot, and both refuse rather than truncate. The trade is a bit per character against the whole repertoire, which is the format's decision and not the codec's.
