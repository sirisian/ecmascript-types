...
A destructuring catch parameter is checked by the same rules as other binding patterns. A clause-level filter supplies the source type for the pattern's member contributions within that clause; a catch without a filter has an unknown source. Written member annotations, defaults and rest annotations are checked even when the handler is never selected. This includes a known noniterable array-pattern source and a known nullish object-pattern source. Each clause has its own bindings. The filter neither converts the thrown value nor infers a type from the try body's throw sites; ordered filtering, untyped fallbacks, property/iterator effects and reference liveness keep their existing rules.
