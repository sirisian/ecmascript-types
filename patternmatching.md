...
Element positions type from the selected iteration contract, using the same static iteration contribution as value spreads. A declared iterator protocol can supply a yielded element type; a guaranteed sequence supplies the type at each position. An immutable tuple Composite supplies its fixed element sequence. A mutable array or tuple's indexed storage does not constrain a replaceable `Symbol.iterator`, so its stored element types and extent alone establish neither yielded types nor a yielded count. Unknown iteration leaves an unannotated pattern binding's type unknown; an explicit annotation such as ```let x: uint8``` still tests and narrows the yielded value. A rest binding collects the remaining yielded values, with their type known only to the extent the iteration contract establishes it.

The pattern still consumes and memoizes the selected iterator, performs its length test, and fails to match when that test or a sub-pattern fails. Those operations retain their effects and ordering. Matching a length does not convert a mutable array into a fixed-length array type, and testing yielded values does not change the types of its indexed storage. For example, a tuple storing a String can have an iterator yielding a ```uint8```: its unannotated pattern element must not inherit ```string``` from the stored slot. The later use of a yielded value at a typed boundary retains a runtime check whenever the iteration contract did not prove its type.
...
The last two are *derived* forms, and derived for this check alone: neither changes what the type is. ```Reflect.typeOf``` reports the dependent record type and the intersection, identity and interning are untouched, and assignability compares against the type as written. All they change is that the checker sees the cases the reader sees, which is the whole job of the check. An arm covers a member when its unguarded pattern provably matches every value of it: a type pattern naming the member or a supertype, an enumerator constant for that enumerator, a binding, an object pattern whose every sub-pattern covers the member's corresponding field, or an array pattern whose length and sub-patterns cover a guaranteed sequence from the member's iteration contract - the subsumption the impossible-test rule already computes, run in the other direction - and a guard forfeits the arm's contribution, since the checker does not evaluate guards. That is what checks the opening example, a union of object types discriminated by a field, and what makes ```Node | null``` - the shape every ```tryParse``` in this proposal returns - exhaustive from ```when null:``` plus the subclass arms, with no ```default``` standing in for the case the types already name.
