...
An arm body is an expression, a ```throw``` expression, or a block. ```throw``` is admitted as an arm rather than left to a library's always-throwing helper, because an arm that reports an impossible case is the commonest arm a total ```match``` has. A block arm's value is the value of its final statement where that statement is an expression statement, and a block arm whose final statement is anything else - a declaration, an ```if```, an empty block - is ```void```, so reading the value of a ```match``` that can take such an arm is the ordinary error of reading a ```void```. The subject is evaluated once, before any pattern.
